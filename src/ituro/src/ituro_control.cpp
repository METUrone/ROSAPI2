...
{   // Ros için gerekli olan şeyler
    ...
    ros::Publisher vis_odom_pub = n.advertise<geometry_msgs::PoseStamped>("/mavros/vision_pose/pose",10);
    ...
    while(ros::ok()){
        ...
        uav_pose.pose.position.z = odom.pose.pose.position.z;
        uav_pose.pose.orientation = odom.pose.pose.orientation;
        vis_odom_pub.publish(uav_pose);
        ros::spinOnce();
        looprate.sleep();
    }
}